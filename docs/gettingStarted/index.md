...
from a template. This template provides a sample customers management service.
...
!!!warning
    By default, data are stored in memory and will be lost if you restart the container.
...
The first thing to do is to create a model. A model (or schema) describes data manipulated by the service. It has many
functionalities:

- For input data, it is used to check and validate argument values (rasing a 400 bad request error if any errors)
- For storing data, it can be used by the **default** provider to get some persistance informations like key definition, uniqueness, collection name...
- For metadata description, it is used to display informations (with /api/_servicedescription) used for code generation or other tools.
...
This is a very basic model, it has the same properties than customer with an additional fullName property.
Since this model will be used for output response, no validations are added.

Now we will create a handler to query a customers view.
...
- A handler is just a class with annoted methods. This annotations describe how the handler can be requested. There are 3 kinds of handler: ```Query handler```, ```Action handler``` and ```Event handler```. See [concepts](../reference/index)
...
- The ```Query``` annotation is used to expose a method. Annotations has mandatory properties used to describe the service. The query will be accessible from ```/api/customersview``` (The name is inferred from the method name removing the Async suffix).
- The handler code is like any traditional method. With some specifications:
    ...
    - The return value must be described by a model (can be the same as the input argument). This is used to decribe the service.
    - If any error occurs, a 500 http error will be send to the client.

You can note that the service use a custom service class (```CustomersService```) to read all customers. This is a proxy class requesting our first service : customers-service.

But how can i create this proxy class ?

### Communicate between services

Thanks to the metadata description hability of any vulcain service, we can used tool to generate code. The code generation uses template to generate a static class with typed method sending fully hight tolerance, monitored and vulcain compatible requests.

Vulcain compatible means context propagation : security context, global correlation id, tenant information...

To generate the proxy class, we need some informations:
...
Then open the dashboard on ```http://localhost:9999/hyxtrix```, set the service endpoint whith ```http://<host ip>:8080/hystrix.stream``` and click on __Monitor Stream__.
...
!!!info
    ...
    If the service were deployed as a swarm service, you can access it with a standardize DNS name corresponding to the service name + its version (removing all special characters).
...
For this situation, **vulcain** uses a hystrix implementation of command. You can configure the command behavior with annotation. For example, the following command has a 1500ms timeout protecting from too long request.

This is the microservice philosophy : Service can failed but quickly.

For many usages, if the request failed it's better to send a default response than an error message. This is the role of the compensation (fallBack) method.
...
If you need to customize a call, you need to customize a command but only for this specific call. To do that, the proxy class will check for every call if a specific command exists for this call else it will use the default command. A specific command is identify by a name like ```<service name><method name>Command```.

So since we will customize a call to ```getAllCustomerAsync``` of the proxy class named ```CustomersService``` we need to create a new command named ```CustomersServiceGetAllCustomer``` (Async is always removed for naming).
...
Annotations are copied from the default generated command. You can see how setting timeout delay and how define informations used for describing the service.

Fallback method is called on every error during the request including timeout.

Now if you request the facade service, the compensation response will be return.
